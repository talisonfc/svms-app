...
export class VendaComponent implements OnInit {
  ...
  searchCliente(event){
    ...
    if(event.keyCode === 13 ){
      this.http.get<Cliente[]>(urlapi+'/clientePorNome/'+this.opcao).subscribe(res=>{
        //console.log(res);
        if(res.length > 0){
          this.options = res;
          /*
  ...
  searchProduto(event){
    ...
    if(event.keyCode === 13){
      this.http.get<Produto[]>(urlapi+'/produtoPorNome/'+this.opcaoProduto).subscribe(res=>{
        //console.log(res);
        if(res.length > 0){
          this.optionsP = res;
          if(res[0].id_sensor !== ""){
            this.http.get<Sensor>(urlapi+'/sensor/'+res[0].id_sensor).subscribe(res=>{
  ...
  ngOnInit() {
    ...
    */
    ...
    this.http.get<Venda[]>(urlapi+'/vendas').subscribe(res=>{
      this.vendas = res;
    })
  }
  ...
  */
  ...
  finalizar(){
    ...
    this.http.post<Venda>(urlapi+'/venda', this.venda).subscribe(res => {
      //console.log(res)
